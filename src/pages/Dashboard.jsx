...
const Dashboard = () => {
  ...
  useEffect(() => {
    if (!user?.token) return; // Wait for token to be available
    ...
    const fetchRecent = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_BASE_URL}/api/report/recent`,
          {
            headers: {
              Authorization: `Bearer ${user.token}`,
            },
  ...
  const exportExcel = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/api/report/export`,
        {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
